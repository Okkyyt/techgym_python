...
def create_cards():
    ...
    for i  in marks:
        for j , num in enumerate(display_name):
            ...
            if j >= 9:
                j = 9
            cards.append(Card(i,num,j+1,card_images[j],0))

def deal_card(player):
    random_number = cards[random.randint(0,51)]
    while random_number.flag == 1:
      random_number = cards[random.randint(0,51)]
    player.cards = random_number
    random_number.flag = 1
